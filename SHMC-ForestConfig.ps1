...
DcPreInstall($name,$IP)

Install-ADDSDomain -DomainType TreeDomain -InstallDns `
    -DomainMode Win2012R2 -NewDomainName massageandspa.ca -NewDomainNetbiosName MS `
    -ParentDomainName shmc.ca 
...
Enable-ADOptionalFeature 'Recycle Bin Feature' -Scope ForestOrConfigurationSet  `
    -Target 'shmc.ca' -server shmc-dc1

New-GPO -Name "Remote Management Automation" -StarterGpoName "Group Policy Remote Update Firewall Ports" `
     | New-GPLink -Target "DC=shmc,dc=ca" -LinkEnabled yes
